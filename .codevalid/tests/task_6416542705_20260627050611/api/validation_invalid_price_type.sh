...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="seller-invalid-price-${CASE_SUFFIX}"
SELLER_PROFILE_ID="sp-invalid-price-${CASE_SUFFIX}"
EMAIL="seller-invalid-price-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/validation_invalid_price_type_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/validation_invalid_price_type_${CASE_SUFFIX}.status"
...
# When — submit a payload with a non-numeric price_cents value
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X POST "$BASE_URL/products" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $TOKEN" \
  --data '{"title":"Invalid Price","description":"Invalid price test","category":"Test","price_cents":"free","stock_qty":5,"photos":[]}' > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:validation_invalid_price_type"
