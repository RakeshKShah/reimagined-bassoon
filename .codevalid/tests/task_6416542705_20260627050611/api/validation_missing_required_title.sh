...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="seller-missing-title-${CASE_SUFFIX}"
SELLER_PROFILE_ID="sp-missing-title-${CASE_SUFFIX}"
EMAIL="seller-missing-title-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/validation_missing_required_title_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/validation_missing_required_title_${CASE_SUFFIX}.status"
...
# When — submit a payload missing title
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X POST "$BASE_URL/products" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $TOKEN" \
  --data '{"description":"Missing title test","category":"Test","price_cents":1000,"stock_qty":5,"photos":[]}' > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:validation_missing_required_title"
