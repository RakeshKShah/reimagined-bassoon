...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="seller-zero-stock-${CASE_SUFFIX}"
SELLER_PROFILE_ID="sp-zero-stock-${CASE_SUFFIX}"
EMAIL="seller-zero-stock-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/edge_case_zero_stock_sold_out_status_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/edge_case_zero_stock_sold_out_status_${CASE_SUFFIX}.status"
PRODUCT_TITLE="Out of Stock Item ${CASE_SUFFIX}"
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_PROFILE_ID}', '${USER_ID}', 'Zero Stock Store ${CASE_SUFFIX}', 'Zero stock bio');"

# When — create a product with zero stock quantity
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X POST "$BASE_URL/products" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $TOKEN" \
  --data '{"title":"'"$PRODUCT_TITLE"'","description":"Currently unavailable","category":"Test","price_cents":5000,"stock_qty":0,"photos":[]}' > "$STATUS_FILE"

# Then — verify product is created as SOLD_OUT
...
echo "CODEVALID_TEST_ASSERTION_OK:edge_case_zero_stock_sold_out_status"

# Cleanup — handled by trap to delete created product, profile, and user
