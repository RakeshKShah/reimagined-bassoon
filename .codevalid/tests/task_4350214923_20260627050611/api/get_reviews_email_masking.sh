...
CASE_SUFFIX="$(date +%s)-$$"
PRODUCT_ID="prod-222-${CASE_SUFFIX}"
...
REVIEW_ID="review-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/get_reviews_email_masking_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/get_reviews_email_masking_${CASE_SUFFIX}.status"
...
INSERT INTO users (id, email, password_hash, role, status, created_at) VALUES
  ...
  ('${BUYER_ID}', 'johnsmith@email.com', 'hash', 'BUYER', 'ACTIVE', NOW());

INSERT INTO seller_profiles (id, user_id, store_name, bio)
VALUES ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Masking Store ${CASE_SUFFIX}', '');

INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at)
VALUES ('${PRODUCT_ID}', '${SELLER_PROFILE_ID}', 'Masking Product', 'Product for masking test', 'general', 2100, 3, '[]', 'ACTIVE', true, NOW());

INSERT INTO orders (id, buyer_id, status, total_cents, platform_fee_cents, created_at)
VALUES ('${ORDER_ID}', '${BUYER_ID}', 'DELIVERED', 2100, 210, NOW());

INSERT INTO order_items (id, order_id, product_id, seller_id, qty, price_at_purchase, seller_payout_cents)
VALUES ('${ORDER_ITEM_ID}', '${ORDER_ID}', '${PRODUCT_ID}', '${SELLER_PROFILE_ID}', 1, 2100, 1890);

INSERT INTO reviews (id, order_item_id, product_id, buyer_id, rating, body, created_at)
VALUES ('${REVIEW_ID}', '${ORDER_ITEM_ID}', '${PRODUCT_ID}', '${BUYER_ID}', 4, 'Solid item', '2024-02-02T10:00:00Z');
SQL
...
[ "$STATUS" = "200" ]
jq -e 'length == 1 and .[0].buyer_email == "jo***@email.com"' "$RESPONSE_FILE" >/dev/null
jq -e '.[0].buyer_email != "johnsmith@email.com"' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:get_reviews_email_masking"
