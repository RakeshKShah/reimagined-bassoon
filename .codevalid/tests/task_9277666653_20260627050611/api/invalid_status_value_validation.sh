...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-invalid-status-${CASE_SUFFIX}"
SELLER_ID="seller-invalid-status-${CASE_SUFFIX}"
EMAIL="invalid-status-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/invalid_status_value_validation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/invalid_status_value_validation_${CASE_SUFFIX}.status"
cleanup_files() {
...
# Given — create a seller that should remain unchanged after validation failure
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Invalid Status Store ${CASE_SUFFIX}', 'Validation target');"

# When — submit an invalid status enum value
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X PUT "$BASE_URL/sellers/${SELLER_ID}" \
  -H 'Content-Type: application/json' \
  --data '{"status":"INVALID_STATUS"}' > "$STATUS_FILE"

# Then — verify validation error and no DB mutation
