...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="user-buyer-10-${CASE_SUFFIX}"
BUYER_EMAIL="checkout-fee-${CASE_SUFFIX}@example.com"
SELLER_USER_ID="seller-user-10-${CASE_SUFFIX}"
SELLER_EMAIL="checkout-fee-seller-${CASE_SUFFIX}@example.com"
SELLER_ID="seller-10-${CASE_SUFFIX}"
PRODUCT_ID="prod-fee-test-11-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/checkout_fee_calculation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/checkout_fee_calculation_${CASE_SUFFIX}.status"
...
INSERT INTO sellers (id, user_id, store_name) VALUES ('${SELLER_ID}','${SELLER_USER_ID}','Checkout Fee Store ${CASE_SUFFIX}');
INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, visible, status)
VALUES ('${PRODUCT_ID}','${SELLER_ID}','Fee Test ${CASE_SUFFIX}','Fee calculation verification product','widgets',10000,100,TRUE,'ACTIVE');
SQL
...
jq -e '
  .status == "PAID" and
  .totalCents == 10000 and
  (.platformFeeCents | type) == "number" and
  .platformFeeCents > 0 and
  (.items[0].sellerPayoutCents | type) == "number" and
  .items[0].sellerPayoutCents > 0 and
  (.platformFeeCents + .items[0].sellerPayoutCents) == 10000
